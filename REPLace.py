...
includes.append('esp_ap.py')
includes.append('uping.py')
includes.append('esp_client.py')
    ...
    def upload(self):
        ...
        for root,dirs,files in os.walk(self.file_system_dir):
            ...
            for file in files:
                ...
                infile = open(path1)
                path2 = os.path.join(tmp_system_dir, 'smash_'+file)
                # smash files
                ...
                # send temp file
                if not smash_only:
                    path3 = path1.replace(self.file_system_dir, '').lstrip('/').lstrip('\\')
